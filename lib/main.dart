...
class _SheetsUploaderState extends State<SheetsUploader> {
  ...
  Future<void> upload() async {
    ...
    final rawValue = dataCtrl.text.trim();
    final uploader = usernameCtrl.text.trim();

    if (row == null || row <= 0 || col.isEmpty || rawValue.isEmpty || uploader.isEmpty) {
      _snack('Please fill in all fields correctly', error: true);
      return;
    }
    ...
    try {
      ...
      // Check existing data
      final existing = await api.spreadsheets.values.get(spreadsheetId, range);
      ...
      if (hasData) {
        _snack('Cell $range already contains data', error: true);
        return;
      }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            const Text(
              'Enter your name, row, column, and value to upload to the Google Sheet.',
            ),
            TextField(
